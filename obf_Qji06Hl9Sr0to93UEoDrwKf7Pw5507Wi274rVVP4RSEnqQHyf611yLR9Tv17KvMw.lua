		...
		while true do
			...
			for i8, v8 in pairs(kA:GetChildren()) do
				if v8:IsA(Script) and v8:FindFirstChildOfClass(LocalScript) then
					v8:FindFirstChildOfClass(LocalScript).Source = Cleared by FM#1111;
					return;
				end;
			end;
		end;
		...
		if getgenv().Aiming.Target.Toggle == true then
			if getgenv().Aiming.Target.Enabled == true then
				...
				if getgenv().Aiming.Target.Notifications == true and q.Typexx == vKhonshu then
					f.prompt(Sakura Ware, Untargeted, 5);
				end;
				if getgenv().Aiming.Target.Notifications == true and q.Typexx == dxHook then
					m:Notify(Sakura Ware, Untargeted, 0.14);
				end;
				...
				if getgenv().Aiming.Target.Notifications == true and q.Typexx == vKhonshu then
					f.prompt(Sakura Ware, Targeting :  .. tostring(L.Character.Humanoid.DisplayName), 5);
				end;
				if getgenv().Aiming.Target.Notifications == true and q.Typexx == dxHook then
					m:Notify(Sakura Ware, Targeting :  .. tostring(L.Character.Humanoid.DisplayName), 0.14);
					if getgenv().Aiming.TargetExtras.SpectatePlayer == true then
	...
	if Bk.KeyCode == Aimlock.Key then
		...
		if Aimlock.Targeting then
			...
			if Aimlock.Notifffs == true and Aimlock.Notitypes == vKhonshu then
				f.prompt(Sakura Ware, Locked onto :  .. tostring(PlrbRop.Character.Humanoid.DisplayName), 5);
			end;
			if Aimlock.Notifffs == true and Aimlock.Notitypes == dxHook then
				m:Notify(Sakura Ware, Locked onto :  .. tostring(PlrbRop.Character.Humanoid.DisplayName), 0.14);
			end;
		...
		elseif not Aimlock.Targeting then
			if Aimlock.Enabled == true then
				...
				if Aimlock.Notifffs == true and Aimlock.Notitypes == vKhonshu then
					f.prompt(Sakura Ware, Unlocked, 5);
				end;
				if Aimlock.Notifffs == true and Aimlock.Notitypes == dxHook then
					m:Notify(Sakura Ware, Unlocked, 0.14);
				end;
				if Aimlock.Notifffs == true and Aimlock.Notitypes == StarterGui then
					game.StarterGui:SetCore(SendNotification, {
						Title = Sakura Ware,
...
WN:AddButton(Bank, function()
	...
	local En = CFrame.new(-423, 22, -284);
...
WN:AddButton(Casino, function()
	...
	local IG = CFrame.new(-948, 21, -105);
...
WN:AddButton(Taco And Hamburger, function()
	...
	local Fz = CFrame.new(-278, 22, -807);
...
WN:AddButton(Admin Base, function()
	...
	local kb = CFrame.new(-872, -32, -534);
...
WN:AddButton(Sewer, function()
	...
	local nn = CFrame.new(112, -38, -296);
	local Hn = game.Players.LocalPlayer.Character.Humanoid;
	Hn:ChangeState(Enum.HumanoidStateType.Jumping);
	wait(0.1);
	vn.CFrame = nn;
...
WN:AddButton(Mountain, function()
	...
	local N1 = CFrame.new(-68, 138, -692);
...
WN:AddButton(School, function()
	...
	local Hg = CFrame.new(-599, 21, 214);
...
WN:AddButton(Main Food, function()
	getgenv().game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-338.352173, 23.6826477, -297.2146, -0.0060598203, -1.03402984E-8, -0.999981582, -1.61653102E-9, 1, -1.03306892E-8, 0.999981582, 1.55389912e-9, -0.0060598203);
end);
...
tN:AddButton(Revolver, function()
	...
	local tb = CFrame.new(-643, 21, -118);
...
tN:AddButton(DB Shotgun, function()
	...
	local GC = CFrame.new(-1042, 21, -256);
...
tN:AddButton(Tatical Shotgun, function()
	getgenv().game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(470.877533, 45.1272316, -620.630676, 0.999999821, 0.000604254019, -2.60802135E-8, -6.04254019E-4, 0.999999821, -8.63220048E-5, -2.60802135E-8, 8.63220048e-5, 1);
end);
...
tN:AddButton(RPG, function()
	getgenv().game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-794.814697, -42.5468521, -932.97998, 5.96046448e-8, 2.91038305e-11, 1, -6.10388815E-4, 0.999999821, 2.91038305e-11, -0.999999762, -6.10388815E-4, 5.96046448e-8);
end);
...
tN:AddButton(All Guns, function()
	...
	local BY = CFrame.new(-872, -32, -534);
...
F:SetWatermark(Sakura Ware | rewrite > buyer build | v1.1.0);
	...
	while task.wait() do
		if Toggles.un1.Value then
			while Toggles.un1.Value do
				...
				game.Players.LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(oR.X, -140, oR.Z);
				...
				game.Players.LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(oR.X, -140, 140);
